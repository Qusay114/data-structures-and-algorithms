...
const toLastNames = people => {
  const localArr = people.map(person => person.firstName + ' ' + person.lastName);
  return localArr ;
};
...
const addValues = (arr) => {
  const summation = arr.reduce((sum , value ) => sum += value , 0);
  return summation ;
};
...
const addPurchases = (arr) => {
  const total = arr.reduce((sum , value )=>sum+=value.purchasePrice , 0);
  return total ;
};
...
const countNumberOfElements = (arr) => {
  const count = arr.reduce((sum , value , index)=> index+1 , 0);
  return count ;
};
...
const returnNames = (arr) => {
  const localArr = [];
  arr.reduce((sum , value) => localArr.push(value.name) , '');
  return localArr ;
};
...
const reversedString = (str) => {
  let chars = [... str];
  const reversedStr = chars.reduce((str , char , index ) => str.padStart(index+1,char) );
  return reversedStr ;
};
...
const countNumberOfChildren = (arr) => {
  const totalChilds = arr.reduce((sum , value , index) =>  (value.children)?sum += (value.children).length : sum+=0 , 0);
  return totalChilds ;
};
...
const extractStat = (statName, arr) => {
  const objectStat = arr.reduce((sum , value ) => value.stat.name === statName ? value : null);
  return objectStat ;
};
...
const extractChildren = (arr) => {
  const localArr = arr.filter(value => {
    if(value.name.indexOf('a') !== -1)
      return value.name ;
  });
  const listNames = [];
  localArr.reduce((sum , value) => listNames.push(value.name) , '');
  return listNames;
};
