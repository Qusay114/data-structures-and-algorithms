...
const validatePin = (pin) => {
  const digitCount = String(pin).match(/([0-9])/g) ;
  const checkNonDigit = String(pin).match(/([^0-9])/g);
  return digitCount? ( (digitCount.length === 4 && !checkNonDigit) ? true : false) :false ;
};
...
const validateWord = (word) => {
  ...
  const countChars = word.match(/\w/g);
  return countChars ? ( (countChars.length >= 5 && countChars.length < 10) ? true : false ) : false ;
};
...
const validatePhoneNumber = (phoneNumber) => {
  const result = phoneNumber.match(/\(?([0-9]{3})\)?[-.●]?[\s]?([0-9]{3})[-.●]?[\s]?([0-9]{4})/g);
  if(!result) return false ;
  if(result[0].length !== phoneNumber.length ) return false ;
  return result[0].match(/\(/) ? (result[0].match(/\)/)? true : false) : true ;
};
