...
const sortByChildren = (charArray) => {
  charArray.sort((fstObj , scdObj) => {
    if((fstObj.children.length - scdObj.children.length) !== 0 )
      return (fstObj.children.length - scdObj.children.length) ;
    else {
      for(let i = 0 ; i<fstObj.house.length ; i++ )
        if((fstObj.house[i].charCodeAt() - fstObj.house[i].charCodeAt()) !== 0)
          return fstObj.house[i].charCodeAt() - fstObj.house[i].charCodeAt() ;
    }
    return 0 ;
  }
  );
...
const citiesAtoJ = (arr) => {
  const results = arr.filter(city => city[0].match(/[A-J]/));
  return results ;
};
