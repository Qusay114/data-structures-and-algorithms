...
const updateAnimal = (arr, callback) => {
  const localArr = [];
  arr.forEach( value => localArr.push(callback(value)));
  return localArr ;
};
...
const alphabetizeBetter = (arr) => {
  arr.sort( (fstEl , scdElm) => {
    for(let i in fstEl)
    {
      const sortIndex = (fstEl.toLowerCase()).charCodeAt(i) - (scdElm.toLowerCase()).charCodeAt(i) ;
      if(sortIndex !== 0 )
        return sortIndex ;
    }
  });
...
const sortByLength = (arr) => {
  arr.sort((fstEl , scdEl) => fstEl.length - scdEl.length);
  return arr ;
};
...
const sortNumbersByLength = (arr) => {
  arr.sort((fstEl , scdEl) => String(fstEl).length - String(scdEl).length) ;
  return arr ;
};
...
const sortPeople = (arr) => {
  arr.sort( (fstEl , scdElm) => {
    for(let i in fstEl.lastName)
    {
      const sortIndex = fstEl.lastName.charCodeAt(i) - scdElm.lastName.charCodeAt(i) ;
      if(sortIndex !== 0 )
        return sortIndex ;
    }
  });
...
const sortPeopleBetter = (arr) => {
  arr.sort( (fstEl , scdElm) => {
    ...
    for(let i in fstEl.firstName)
    {
      const sortIndex = fstEl.firstName.charCodeAt(i) - scdElm.firstName.charCodeAt(i) ;
      if(sortIndex !== 0 )
        return sortIndex ;
    }

    return fstEl.age - scdElm.age ;
...
const sortMeetingsByDay = (arr) => {
  arr.sort( (obj1 , obj2) => dayIndex(obj1.dayOfWeek) - dayIndex(obj2.dayOfWeek) );
  return arr ;
};
...
const sortSchedule = (arr) => {
  const localArr = sortMeetingsByDay(arr);
  localArr.sort((a,b) => {
