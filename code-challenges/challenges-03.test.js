...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  const localArr = arr.filter(statsObj => {
    return(statsObj.baseStat > minBaseStat);
  });
  return localArr ;
};
...
const getStatName = (arr, minBaseStat) => {
  const localArr = getBaseStatGreaterThan(arr,minBaseStat).map(statObj => {
...
const getCharactersWithoutChildren = (arr) => {
  const localArr = arr.filter(charObj => {
    return(!charObj.children);
  });
  return localArr ;
};
...
const evenOddNumericValues = (arr) => {
  ...
  const localArr = filteredArr.map(num => {
    if(num % 2 === 0)
      return 'even';
    else if(num % 1 === 0)
      return 'odd';
